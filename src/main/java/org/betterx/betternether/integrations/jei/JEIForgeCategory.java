...
public class JEIForgeCategory implements IRecipeCategory<ForgeDisplay> {
    ...
    public JEIForgeCategory(IGuiHelper guiHelper) {
        ResourceLocation vanillaFurnace = ResourceLocation.fromNamespaceAndPath(
                "minecraft",
    ...
    @Override
    public @NotNull RecipeType<ForgeDisplay> getRecipeType() {
    ...
    @Override
    public void createRecipeExtras(IRecipeExtrasBuilder builder, ForgeDisplay recipe, IFocusGroup focuses) {
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, ForgeDisplay display, IFocusGroup focuses) {
        builder.addSlot(RecipeIngredientRole.INPUT, 1, 1).addIngredients(display.getInput());
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level != null) {
            ItemStack result = display.getResultItem(minecraft.level.registryAccess());
            if (!result.isEmpty()) {
                builder.addSlot(RecipeIngredientRole.OUTPUT, 61, 19).addItemStack(result);
            }
        }
    }
