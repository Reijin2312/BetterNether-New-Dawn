...
public class CrystalFeature extends NetherSurfaceFeature {
    ...
    protected void generate(
        ...
        final int index = random.nextInt(PALETTE_IDS.length >> 1);
                    ...
                    if (d <= 0) {
                        ...
                        if (d <= -0.3) {
                            state = random.nextInt(12) == 0 ? (isBlue
                                    ? blockFromId(BLUE_WEEPING_OBSIDIAN_ID, Blocks.OBSIDIAN)
                                    : blockFromId(WEEPING_OBSIDIAN_ID, Blocks.OBSIDIAN)).defaultBlockState() : getState(index, v);
                        } else if (d <= -0.15) {
                            state = random.nextInt(9) == 0 ? (isBlue
                                    ? blockFromId(BLUE_CRYING_OBSIDIAN_ID, Blocks.CRYING_OBSIDIAN)
                            ...
                            state = random.nextInt(4) == 0
                                    ? (isBlue
                                    ? blockFromId(BLUE_WEEPING_OBSIDIAN_ID, Blocks.OBSIDIAN)
                                    : blockFromId(WEEPING_OBSIDIAN_ID, Blocks.OBSIDIAN)).defaultBlockState()
                                    : (isBlue
                                            ? blockFromId(BLUE_CRYING_OBSIDIAN_ID, Blocks.CRYING_OBSIDIAN)
    ...
    private BlockState getState(int index, Vec3 pos) {
        ...
        return paletteState(blockIndex);
    }
